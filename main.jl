...
plot_phase("$(p.filename)1.h5", color="b")
plot_phase("$(p.filename)$(p.nfiles).h5", color="r")
plt.axis("equal")
plt.savefig("./figures/$(p.filename).pdf", bbox_inches="tight")
plt.show()
plot_E("$(p.filename)1.h5", color="b")
plot_E("$(p.filename)$(p.nfiles).h5", color="r")
